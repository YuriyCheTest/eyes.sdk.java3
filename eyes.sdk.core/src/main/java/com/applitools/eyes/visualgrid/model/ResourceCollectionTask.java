...
public class ResourceCollectionTask implements Callable<TestResultContainer> {
            ...
            if (result[i] != null && result[i]) {
                synchronized (uploadedResourcesCache) {
                    if (resourceUrl.equals(domResource.getUrl())) {
                        domResource.resetContent();
                        uploadedResourcesCache.put(domResource.getSha256(), null);
                    } else {
                        RGridResource resource = resourceMap.get(resourceUrl);
    ...
    void uploadResources(Map<String, RGridResource> resources) {
        for (String url : resources.keySet()) {
            ...
            synchronized (resourcesCacheMap) {
                if (!resourcesCacheMap.containsKey(url)) {
                    resource = resources.get(url);
                } else {
                    resource = resourcesCacheMap.get(url);
                }
            }
            ...
            SyncTaskListener<Void> listener = new SyncTaskListener<>(logger, "uploadResource");
        ...
        for (String url : resources.keySet()) {
            ...
            // A blocking call
            uploadedResourcesCache.get(resource.getSha256()).get();
        }
    }
}
